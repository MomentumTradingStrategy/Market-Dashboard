...
def _heat_rs(v):
    ...
    x = (v - 1) / 98.0
    # Red -> Amber -> Green
    # we build an RGB that looks like your Excel scale
    if x < 0.5:
        # red to amber
        r = 255
...
def right_panel_ui():
    ...
    st.markdown(
        '<div class="card"><div style="font-weight:900; font-size:1.0rem;">Manual Inputs (Top Right)</div>'
...
with left_col:
    ...
    with c1:
        st.dataframe(style_grouped(df_left[show_cols]), use_container_width=True, height=980)
    with c2:
        st.dataframe(style_grouped(df_right[show_cols]), use_container_width=True, height=980)
